...
export const SharePortalsModal: React.FC<Props> = ({ isOpen, onClose, initialPortalId }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-4xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200/90 dark:border-slate-700/80 rounded-2xl p-5 space-y-5">
            ...
            {/* QR Code Section */}
            <div className="pt-4 border-t border-slate-200/80 dark:border-slate-700/60 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-xl border">
              <div className="flex items-center gap-4">
                {qrCodeDataUrl ? (
                  <img
                    src={qrCodeDataUrl}
                    alt={`QR Code ${activePortal.title}`}
                    className="w-20 h-20 rounded-lg border border-slate-200 dark:border-slate-700 p-1 bg-white shrink-0 shadow-2xs"
                  />
                ) : (
                  <div className="w-20 h-20 rounded-lg bg-slate-100 dark:bg-slate-800 animate-pulse shrink-0"></div>
                )}
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 max-w-sm">
                    Imprima ou envie o QR Code para os usuários escanearem com a câmera do celular no campo ou nas escolas.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-initial px-3.5 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-700 flex items-center justify-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
                >
